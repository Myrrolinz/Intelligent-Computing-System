...
NKU 智能计算系统 AICS final

该仓库为caffe2darknet的思路参考，实际在darknet可以进行推理但是结果有问题，思路可参考报告部分。

## 任务一：利用工具链实现模型部署

实现torch2caffe, torch2onnx的转化

## 任务二：使用模型转换工具将预训练模型转换至 Darknet

转换的模型为Resnet18，具体思路见报告。本目录中的2-1,2-2,2-3分别存储三个实验思路。

参考过https://github.com/leiyu1980/caffe-darknet-convert/tree/master（但是仍然是错误结果）
# convert resnet18
```
